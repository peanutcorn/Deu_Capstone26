...
def main():
    ...
    model = eval(cfg.MODEL.NAME)(
        backbone=cfg.MODEL.PRETRAINED_MODEL, 
        dataset=cfg.DATASET, 
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        if epoch == 0:
            # MSP
            tmp_model = os.path.join(
                final_output_dir, 'tmp', 'model_state_initial.pth'
            )
        ...
        # MSP
        tmp_model = os.path.join(
            final_output_dir, 'model_state_' + str(epoch) + '.pth'
        )
        torch.save(model.module.state_dict(), tmp_model)
        ...
        # MSP
        tmp_model = os.path.join(
            final_output_dir, 'tmp', 'model_state_' + str(epoch)
        )
        torch.save(model.module.state_dict(), tmp_model)
