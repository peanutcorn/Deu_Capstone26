...
class plt_config:
    def __init__(self, dataset_name):
        if dataset_name == 'coco':
            ...
            self.n_kpt = 16
            self.EDGES = [
                ([0, 1], [255, 0, 0]),          # r_ankle -> r_knee
                ([1, 2], [155, 85, 0]),         # r_knee -> r_hip
                ([2, 6],  [155, 85, 0]),        # r_hip -> pelvis
                ([5, 4], [0, 0, 255]),          # l_ankle -> l_knee
                ([4, 3], [17, 25, 10]),         # l_knee -> l_hip
                ([3, 6],  [0, 0, 255]),         # l_hip -> pelvis
                ([6, 7],   [0, 255, 0]),        # pelvis -> chest
                ([7, 8],   [0, 255, 5]),        # chest -> neck
                ([8, 9],   [0, 255, 170]),      # neck -> head(top)
                ([10, 11],   [0, 255, 25]),     # r_wrist -> r_elbow
                ([11, 12],   [0, 17, 255]),     # r_elbow -> r_shoulder
                ([12, 7],   [0, 220, 0]),       # r_shoulder -> chest
                ([15, 14],   [0, 220, 0]),      # l_wrist -> l_elbow
                ([14, 13],   [125, 125, 155]),  # l_elbow -> l_shoulder
                ([13, 7],   [25, 0, 55]),       # l_shoulder -> chest
            ]
        else:
            raise ValueError(
        ...
        for j in range(len(skeletons)):
            if len(skeletons[j][i]) > 2 and skeletons[j][i, 2] > 0:
                cv2.circle(canvas, tuple(skeletons[j][i, 0:2].astype(
                    'int32')), 10, (255,255,255), thickness=-1)
...
# by MSP
def convert_dstKeypoints_orgKeypoints(keypoints_dst, img_org, img_dst):
    org_h, org_w = img_org.shape[:2]
    dst_h, dst_w = img_dst.shape[:2]
    
    keypoints_org = keypoints_dst.copy()
    keypoints_org[:, 0] = keypoints_org[:, 0] * org_w / dst_w 
    keypoints_org[:, 1] = keypoints_dst[:, 1] * org_h / dst_h
    keypoints_org = keypoints_org.astype(int)
    
    return keypoints_org
