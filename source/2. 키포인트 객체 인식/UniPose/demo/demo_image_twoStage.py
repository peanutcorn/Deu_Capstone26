...
# argument for image
parser.add_argument('--image-folder', type=str, default="./data", help='image folder', required=True)        
...
# argument for saving txt
parser.add_argument('--flag-save', type=bool, default=False, help='save "bboxes"/"keypoints" in txt file')        
...
assert os.path.exists(args.image_folder), "Check video folder : %s" %args.image_folder
...
image_list = [image for image in image_list if '.jpg' in image.lower() or '.png' in image.lower()]
...
with torch.no_grad():
    ...
    for img_path in image_list:
        ...
        img = cv2.imread(os.path.join(args.image_folder, img_path), flags=cv2.IMREAD_COLOR)
        ...
        # save image
        save_image_path = os.path.join(args.save_folder, img_path.split('.')[0] + "_keypoints.jpg")
        cv2.imwrite(save_image_path, img)

        # save txt
        if args.flag_save:
            save_str = str(bboxes) + '\n'
            save_str += str(keypoints)
            
            save_txt_path = os.path.join(args.save_folder, img_path.split('.')[0] + "_keypoints.txt")
